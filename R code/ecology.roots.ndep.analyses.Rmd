---
title: Anthropogenic N deposition slows fine root decay by altering the composition of fungal and bacterial communities
...
This document contains all analyses that accompany the manuscript, "*Anthropogenic N deposition slows fine root decay by altering the composition of fungal and bacterial communities*," by W. A. Argiroff, D. R. Zak, R. A. Upchurch, S. O. Salley, and A. S. Grandy.  
...
knitr::opts_knit$set(root.dir = normalizePath("~/Desktop/Ecology_roots_ndep_analyses"))
...
# Save image
#ggsave("Figure_1.tiff", pyr.gcms.source.fig, device = "tiff", width = 7, height = 8.67, units = "in", dpi = 1200)
...
## A. Conversion to fastq format  

`.bam` files were converted to `.fastq` files using [bedtools v2.27.0.](https://bedtools.readthedocs.io/en/latest/). Code is contained in the file named `bam.to.fastq.txt`. Fungal 28S `.bam` files are available under the accession numbers NUMBERS, and bacterial 16S `.bam` files are available under the accession numbers NUMBERS, on [GenBank](LINK).  
...
Sequences were uploaded to the [RDP classifier (release 11, update 5)](https://rdp.cme.msu.edu/classifier/classifier.jsp) and classified against the Fungal LSU Training Set 11. File headings from the output were then removed and files were concatenated into a single text file with taxonomic assignments. Files were concatenated using the code in the file named `rdp.cat.txt`, and the command `sh rdp.cat.txt`.  Input files and the output file, `Gradient.28S.Roots.combined.trim.unique.fasta_classified.txt`, are included.  
...
#ggsave("Figure_2.tiff", relabund.manuscript.final.barplot, device = "tiff", width = 7, height = 7.3, units = "in", dpi = 1200)
...
f.genus.dbrda.B.fig <- ggplot() + 
  ...
  geom_point(data = f.genus.dbrda.tbl, 
             ...
             alpha = 0) + 
  # Update color
  scale_colour_manual(values = c("green3", "sienna"), 
...
b.family.dbrda.scr.tbl <- dplyr::as_tibble(rownames_to_column(as.data.frame(b.family.dbrda$CCA$v))) %>% 
  ...
  dplyr::mutate(FAMILY.FUNCTION = c("Non-Ligninolytic", "Non-Ligninolytic", "Ligninolytic", 
                                    "Non-Ligninolytic", "Non-Ligninolytic", "Non-Ligninolytic", 
                                    "Non-Ligninolytic", "Non-Ligninolytic", "Non-Ligninolytic", 
                                    "Non-Ligninolytic", "Non-Ligninolytic", "Non-Ligninolytic", 
                                    "Non-Ligninolytic"))
...
#ggsave("4_microbial_composition/Figure_3.tiff", manuscript.dbrda.fig, device = "tiff", width = 10.25, height = 10.25, units = "in", dpi = 1200)
#ggsave("4_microbial_composition/Figure_3.png", manuscript.dbrda.fig, device = "png", width = 10.25, height = 10.25, units = "in", dpi = 300)
```
